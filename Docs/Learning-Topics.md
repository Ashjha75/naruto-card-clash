---
id: learning-topics
title: Naruto Card Clash — Topics You'll Actually Use
type: reference
tags: [learning, java, spring, concurrency, testing, redis]
keywords:
    [
        oop,
        collections,
...
summary: Study guide mapping Java, Spring, database, Redis, concurrency, and testing topics to real parts of the Naruto Card Clash codebase.
---
...
| Composition over inheritance   | `GameSession` _holds_ `List<Card>` rather than extending `Card` — favor "has-a" over "is-a"                                |
...
| `equals`/`hashCode`                        | Needed correctly on any entity you put in a `Set` or use as a `Map` key — Lombok's `@EqualsAndHashCode` handles this, but know _why_ it matters                        |
...
| Authentication vs. Authorization            | Login proves _who_ you are; `@PreAuthorize` decides _what_ you're allowed to do                                    |
